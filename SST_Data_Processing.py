...
# Function to retreive SST data
def get_sst(date_str, lon, lat):
    try:
        ...
        nc_file_path = f"C:\\SST_{year}{month:02d}{day:02d}.nc"
        # Open the NetCDF file
        ...
        target_seconds = (datetime(year, month, day) - datetime(1981, 1, 1)).total_seconds()
        ...
        # Extract the SST value at the determined time, latitude, and longitude indices
        sst_value = nc_data['analysed_sst'][time_index, lat_index, lon_index]
...
df = pd.read_csv(csv_path, encoding='Windows-1252') # the encoding detected earlier

# Create an empty "SST" column in the DataFrame
df['SST'] = np.nan

# Use apply method to fill in the "SST" values from netCDF data
df['SST'] = df.apply(lambda row: get_sst(date_str=row['Date'], lon=row['LONGITUDE'], lat=row['LATITUDE']), axis=1)
